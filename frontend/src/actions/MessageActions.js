        ...
        fetch(IP,
            ...
            .then(json => {
               ...
               // console.log(`chat created with chatKey ==`) || console.log(chatKey);
                ...
                Cookies.set(COOKIE_CHATS, [...Cookies.getJSON(COOKIE_CHATS), {key: chatKey, name: chatName}], { expires: 1000 }); //adding new chatKey to cookie
...
export const leaveChat = (chatKey) => {
    return dispatch => {
        ...
        if (index !== -1 && index !== 0 && index !== 1) { //if user tries to leave from not stored chat, or from initial chats - don't do anything
        ...
        fetch(IP,
            ...
            .then(json => {
               // console.log(`json ==`) || console.log(json);
                ...
                if (chatKeyRes) { //if no chatKey was provided
...
export const getMessages = (lastId, chatKey) => {
    return dispatch => {
        ...
        fetch(IP,
            ...
            .then(json => {
              //  console.log(`json ==`) || console.log(json);
                if (Array.isArray(json) && json.length === 0){ //if no json
                  //  console.log(`no new messages`);
                    dispatch(getMessagesSuccess([], chatKey));
                }else {
                    let newMessages = json.results.items;
                    if (!newMessages || newMessages.length === 0) { //if no new messages
                       // console.log(`no new messages`);
                        dispatch(getMessagesSuccess([], chatKey));
                    } else {
